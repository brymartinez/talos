...
export function FilterBar({
  ...
  return (
    <div className="filter-bar">
      ...
      <label><span className="sr-only">Agent status</span><select value={status} onChange={(event) => onFilter("status", event.target.value)}><option value="">All statuses</option><option value="queued">Queued</option><option value="running">Running</option><option value="needs_input">Needs input</option><option value="failed">Failed</option><option value="succeeded">Succeeded</option></select></label>
